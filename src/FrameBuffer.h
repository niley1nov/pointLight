...
public:
	...
	FrameBuffer(const unsigned int& width,const unsigned int& height,unsigned int attachNum)
	{
		...
		GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGB, GL_FLOAT, NULL));
...
public:
		...
		:m_Count(count), cascadeEnd(CasEnds), ar(arr)
	{
		...
		for (unsigned int i = 0; i < count; ++i)
		{
			...
			GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, width, height, 0, GL_RGBA, GL_FLOAT, NULL));
			GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
			GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
...
private:
	unsigned int m_RendererID;
	unsigned int m_TexMap;
public:
	DepthCube(const unsigned int& width = 1024 , const unsigned int& height=1024)
	{
		GLCall(glGenFramebuffers(1, &m_RendererID));
		GLCall(glBindFramebuffer(GL_FRAMEBUFFER, m_RendererID));

		glGenTextures(1, &m_TexMap);
		GLCall(glBindTexture(GL_TEXTURE_CUBE_MAP, m_TexMap));
		...
		GLCall(glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, m_TexMap, 0));
	...
	void BindTex(unsigned int num = 0)
	{
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, m_TexMap);
	}
	void UnbindTex()
	{
